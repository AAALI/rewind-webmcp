...
1. **Hook:** “I asked for a running outfit under $200. My agent just built a $789 cart.”
2. Show the official Vercel Shop + Shopify example and the user request.
3. Let the agent call `update_cart`; the normal cart opens automatically with eight products.
4. Show the contextual message explaining that the result exceeds the budget by $589.
...
- The Shopify cart demo can surface, explain and undo the over-budget agent action in one recovery click.
